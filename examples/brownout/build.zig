...
// Standalone package for the 'brownout' example (ESP32) — RTC brownout detector.
// Consumes the workspace root (`esp32_hal`). Build-only: QEMU has no supply model.
pub fn build(b: *std.Build) void {
    ...
    mod.strip = true;
    mod.sanitize_c = .off;
    const exe = b.addExecutable(.{ .name = "brownout", .root_module = mod });
