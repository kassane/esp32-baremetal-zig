...
pub fn build(b: *std.Build) void {
    ...
    const hw = firmware(b, core, target, optimize, "esp32_baremetal_zig");
    hw.setLinkerScript(core.namedLazyPath("esp32.ld"));
    b.installArtifact(hw);
    const bin = b.addObjCopy(hw.getEmittedBin(), .{ .format = .bin, .basename = "esp32_baremetal_zig.bin" });
    b.getInstallStep().dependOn(&b.addInstallBinFile(bin.getOutput(), "esp32_baremetal_zig.bin").step);

    // QEMU firmware: all code in IRAM (root's qemu linker), with run + smoke.
    const qemu_exe = firmware(b, core, target, optimize, "esp32_qemu");
    ...
    b.step("run", "Build + run the ESP32 example in QEMU").dependOn(&run.step);
