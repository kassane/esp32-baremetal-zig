...
// Standalone package for the 'gpio_matrix' example (ESP32) — GPIO matrix signal
// routing. Consumes the workspace root (`esp32_hal`). Build-only.
pub fn build(b: *std.Build) void {
    ...
    const target = b.resolveTargetQuery(.{ .cpu_arch = .xtensa, .os_tag = .esp32, .abi = .none });
    ...
    mod.strip = true;
    mod.sanitize_c = .off;
    const exe = b.addExecutable(.{ .name = "gpio_matrix", .root_module = mod });
