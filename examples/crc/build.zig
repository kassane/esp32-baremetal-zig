...
// Standalone package for the 'crc' example (ESP32) — software CRC-32 via
// std.hash.Crc32 (no peripheral, no ROM). QEMU-runnable: wires `run`/`smoke`/`demo`
// on the workspace root (`esp32_hal`).
const machine = "esp32";
...
pub fn build(b: *std.Build) void {
    ...
    const hw = firmware(b, core, target, optimize, "crc");
    hw.setLinkerScript(core.namedLazyPath("esp32.ld"));
    b.installArtifact(hw);
    const bin = b.addObjCopy(hw.getEmittedBin(), .{ .format = .bin, .basename = "crc.bin" });
    b.getInstallStep().dependOn(&b.addInstallBinFile(bin.getOutput(), "crc.bin").step);

    // QEMU firmware: all code in IRAM (root's qemu linker), with run + smoke + demo.
    const qemu_exe = firmware(b, core, target, optimize, "crc_qemu");
    ...
    b.step("run", "Build + run the crc example in QEMU").dependOn(&run.step);
    ...
    b.step("smoke", "Boot the crc example in QEMU and assert no CPU faults").dependOn(&run_smoke.step);
    ...
    b.step("demo", "Run the crc example in QEMU and print its UART output").dependOn(&run_demo.step);
}
