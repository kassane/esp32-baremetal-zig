...
// Standalone package for the 'heap' example (ESP32) — fixed-buffer heap allocation.
// QEMU-runnable (pure SRAM, no peripheral needed): wires `run` / `smoke` / `demo`
...
pub fn build(b: *std.Build) void {
    ...
    const hw = firmware(b, core, target, optimize, "heap");
    hw.setLinkerScript(core.namedLazyPath("esp32.ld"));
    b.installArtifact(hw);
    const bin = b.addObjCopy(hw.getEmittedBin(), .{ .format = .bin, .basename = "heap.bin" });
    b.getInstallStep().dependOn(&b.addInstallBinFile(bin.getOutput(), "heap.bin").step);

    // QEMU firmware: all code in IRAM (root's qemu linker), with run + smoke + demo.
    const qemu_exe = firmware(b, core, target, optimize, "heap_qemu");
    ...
    b.step("run", "Build + run the heap example in QEMU").dependOn(&run.step);
    ...
    b.step("smoke", "Boot the heap example in QEMU and assert no CPU faults").dependOn(&run_smoke.step);
    ...
    b.step("demo", "Run the heap example in QEMU and print its UART output").dependOn(&run_demo.step);
}
...
fn firmware(
    ...
    inline for (.{ "mmio", "heap", "init", "panic", "startup" }) |m| mod.addImport(m, core.module(m));
