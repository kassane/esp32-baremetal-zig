...
// Standalone package for the 'reset_reason' example (ESP32) — read and log the
// PRO-CPU reset cause. Consumes the workspace root (`esp32_hal`). Build-only here,
// but the read path also runs fine in QEMU.
pub fn build(b: *std.Build) void {
    ...
    mod.strip = true;
    mod.sanitize_c = .off;
    const exe = b.addExecutable(.{ .name = "reset_reason", .root_module = mod });
