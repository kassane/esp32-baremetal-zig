...
// Standalone package for the 'gpio_edge' example (ESP32) — poll-based GPIO edge
// detection. Consumes the workspace root (`esp32_hal`). Build-only here, though
// the register path also runs in QEMU.
pub fn build(b: *std.Build) void {
    ...
    mod.strip = false;
    mod.sanitize_c = .off;
    const exe = b.addExecutable(.{ .name = "gpio_edge", .root_module = mod });
