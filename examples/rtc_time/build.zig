...
// Standalone package for the 'rtc_time' example (ESP32) — RTC main-timer uptime.
// QEMU-runnable: wires `run` / `smoke` / `demo` on the workspace root (`esp32_hal`).
const machine = "esp32";
...
pub fn build(b: *std.Build) void {
    ...
    const hw = firmware(b, core, target, optimize, "rtc_time");
    hw.setLinkerScript(core.namedLazyPath("esp32.ld"));
    b.installArtifact(hw);
    const bin = b.addObjCopy(hw.getEmittedBin(), .{ .format = .bin, .basename = "rtc_time.bin" });
    b.getInstallStep().dependOn(&b.addInstallBinFile(bin.getOutput(), "rtc_time.bin").step);

    // QEMU firmware: all code in IRAM (root's qemu linker), with run + smoke + demo.
    const qemu_exe = firmware(b, core, target, optimize, "rtc_time_qemu");
    ...
    b.step("run", "Build + run the rtc_time example in QEMU").dependOn(&run.step);
    ...
    b.step("smoke", "Boot the rtc_time example in QEMU and assert no CPU faults").dependOn(&run_smoke.step);
    ...
    b.step("demo", "Run the rtc_time example in QEMU and print its UART output").dependOn(&run_demo.step);
}
