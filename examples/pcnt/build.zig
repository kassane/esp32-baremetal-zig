...
// Standalone package for the 'pcnt' example (ESP32) — Pulse Counter unit 0.
// Consumes the workspace root (`esp32_hal`). Build-only: QEMU models no PCNT.
pub fn build(b: *std.Build) void {
    ...
    mod.strip = true;
    mod.sanitize_c = .off;
    const exe = b.addExecutable(.{ .name = "pcnt", .root_module = mod });
