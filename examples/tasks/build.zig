...
// Standalone package for the 'tasks' example (ESP32) — a cooperative super-loop
// scheduler (`hal.runTasks`) running two periodic tasks at different rates over
// UART. QEMU-runnable: it wires `run` / `smoke` / `demo` on top of the workspace
// root (`esp32_hal`).
const machine = "esp32";
...
pub fn build(b: *std.Build) void {
    ...
    const hw = firmware(b, core, target, optimize, "tasks");
    hw.setLinkerScript(core.namedLazyPath("esp32.ld"));
    b.installArtifact(hw);
    const bin = b.addObjCopy(hw.getEmittedBin(), .{ .format = .bin, .basename = "tasks.bin" });
    b.getInstallStep().dependOn(&b.addInstallBinFile(bin.getOutput(), "tasks.bin").step);

    // QEMU firmware: all code in IRAM (root's qemu linker), with run + smoke + demo.
    const qemu_exe = firmware(b, core, target, optimize, "tasks_qemu");
    ...
    b.step("run", "Build + run the tasks example in QEMU").dependOn(&run.step);
    ...
    b.step("smoke", "Boot the tasks example in QEMU and assert no CPU faults").dependOn(&run_smoke.step);
    ...
    b.step("demo", "Run the tasks example in QEMU and print its UART output").dependOn(&run_demo.step);
}
