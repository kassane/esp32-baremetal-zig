...
// Standalone package for the 'clock_gate' example (ESP32) — DPORT peripheral
// clock + reset control. Consumes the workspace root (`esp32_hal`). Build-only.
pub fn build(b: *std.Build) void {
    ...
    mod.strip = false;
    mod.sanitize_c = .off;
    const exe = b.addExecutable(.{ .name = "clock_gate", .root_module = mod });
