...
// Standalone package for the 'rtc_store' example (ESP32) — RTC retention scratch
// register round-trip. The Espressif QEMU esp32 machine backs RTC_CNTL.STORE0, so
// this is QEMU-runnable: it wires `run` / `smoke` / `demo` on top of the workspace
// root (`esp32_hal`).
const machine = "esp32";
...
pub fn build(b: *std.Build) void {
    ...
    const hw = firmware(b, core, target, optimize, "rtc_store");
    hw.setLinkerScript(core.namedLazyPath("esp32.ld"));
    b.installArtifact(hw);
    const bin = b.addObjCopy(hw.getEmittedBin(), .{ .format = .bin, .basename = "rtc_store.bin" });
    b.getInstallStep().dependOn(&b.addInstallBinFile(bin.getOutput(), "rtc_store.bin").step);

    // QEMU firmware: all code in IRAM (root's qemu linker), with run + smoke + demo.
    const qemu_exe = firmware(b, core, target, optimize, "rtc_store_qemu");
    ...
    b.step("run", "Build + run the rtc_store example in QEMU").dependOn(&run.step);
    ...
    b.step("smoke", "Boot the rtc_store example in QEMU and assert no CPU faults").dependOn(&run_smoke.step);
    ...
    b.step("demo", "Run the rtc_store example in QEMU and print its UART output").dependOn(&run_demo.step);
}
