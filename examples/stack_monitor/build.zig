...
// Standalone package for the 'stack_monitor' example (ESP32-S3) — ASSIST_DEBUG
// stack-overflow monitor. Consumes the workspace root (`esp32_hal`). Build-only:
// QEMU does not model ASSIST_DEBUG.
pub fn build(b: *std.Build) void {
    ...
    mod.strip = false;
    mod.sanitize_c = .off;
    const exe = b.addExecutable(.{ .name = "stack_monitor", .root_module = mod });
