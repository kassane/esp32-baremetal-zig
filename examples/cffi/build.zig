...
// Standalone package for the 'cffi' example (ESP32) — compiles blob.c to an
// Xtensa object and links it into the firmware (external/precompiled object +
// C-ABI calls). QEMU-runnable: wires `run` / `smoke` / `demo` on the workspace
// root (`esp32_hal`).
const machine = "esp32";
...
pub fn build(b: *std.Build) void {
    ...
    const hw = firmware(b, core, target, optimize, "cffi");
    hw.setLinkerScript(core.namedLazyPath("esp32.ld"));
    b.installArtifact(hw);
    const bin = b.addObjCopy(hw.getEmittedBin(), .{ .format = .bin, .basename = "cffi.bin" });
    b.getInstallStep().dependOn(&b.addInstallBinFile(bin.getOutput(), "cffi.bin").step);

    // QEMU firmware: all code in IRAM (root's qemu linker), with run + smoke + demo.
    const qemu_exe = firmware(b, core, target, optimize, "cffi_qemu");
    ...
    b.step("run", "Build + run the cffi example in QEMU").dependOn(&run.step);
    ...
    b.step("smoke", "Boot the cffi example in QEMU and assert no CPU faults").dependOn(&run_smoke.step);
    ...
    b.step("demo", "Run the cffi example in QEMU and print its UART output").dependOn(&run_demo.step);
}
...
fn firmware(
    ...
    // Compile the C "blob" for the firmware's own target and link the object in.
    const blob_mod = b.createModule(.{ .target = target, .optimize = optimize });
    blob_mod.sanitize_c = .off; // no UBSan runtime on bare metal (same as the firmware)
    blob_mod.addCSourceFile(.{ .file = b.path("blob.c") });
    const blob = b.addObject(.{ .name = "blob", .root_module = blob_mod });
    mod.addObjectFile(blob.getEmittedBin());
    mod.strip = false;
    mod.sanitize_c = .off;
